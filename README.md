<!--
**vbatcnam/vbatcnam** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
-->
## ✨ Salut, explorateurice ! 👋

🌱 **Je teste [FuncSug](https://github.com/cl4cnam/funcSug)**, un langage expérimental conçu pour **simplifier la programmation de la gestion des événements**. 

👯 **Je collabore activement et j’explore ses possibilités**, Je réfléchis à comment il peut faciliter et enrichir la création de **jeux éducatifs** et d’**histoires interactives**.

😄 **FuncSug est né d’une rencontre**, lors de mon stage au CNAM et d’une réflexion autour de **SugarCubesJS**. 

✍️ **Je raconte son histoire dans mon blogue** : [Ticoloupa](https://ticoloupa.blogspot.com/)  

## 🚀  Découvrez FuncSug 

✨ Partez à la découverte de ce nouveau langage et de ses possibilités en naviguant sur **Le site officiel**  [FuncSug](https://funcsug.prefasite.fr/fr/accueil/). J'y ai ajouté des **expériences ludiques et narratives** qui tirent parti de ses fonctionnalités.  

🛠️ Vous pouvez le **tester** grâce au [Playground FuncSug](https://cl4cnam.github.io/try_FuncSug). Des exemples vous guident.

### 📖 Tutoriel FuncSug  
🔹 **Apprenez à utiliser FuncSug étape par étape !**  
💡 J'attends vos retours pour l'améliorer 🚀 
➡️ [Tutoriel FuncSug](https://funcsug.prefasite.fr/fr/tuto/#supportWeb)  

### 📜 Manuel FuncSug (en évolution)  
🔹 **Découvrez le guide officiel de FuncSug**, avec des mises à jour régulières et des explications détaillées.  
🔹 Suivez son développement et apprenez à utiliser les fonctionnalités du langage.   
➡️ [Manuel FuncSug](https://funcsug.prefasite.fr/fr/manuel/co/guide.html)  

💬  **Des suggestions ou des questions ?** Vous pouvez contribuer via [GitHub Issues](https://github.com/vbatcnam/FuncSug/issues) et échanger avec nous ! 🚀 

## 🔥 WANTED : Collaborateurs & Testeurs pour FuncSug  

🚀 **FuncSug cherche des esprits curieux et passionnés !**  
🌍 Un langage **open source et libre**, conçu pour la narration interactive et les jeux éducatifs.  
🛠️ **Votre mission** (si vous l’acceptez) :
> 👉 Tester FuncSug et explorer ses possibilités.  
> 👉 Partager vos idées et vos retours pour l’améliorer.  
> 👉 Contribuer à son développement avec vos compétences en création interactive.  

## 📣 Rejoignez-nous sur les réseaux !  
💬 **Envie d’échanger sur le développement de FuncSug**
🐘[Mastodon-pialle](https://piaille.fr/@cli345)

💬 **Envie d’échanger sur FuncSug et les jeux éducatifs et la narration interactive ?** 
🐘 [Mastodon - Ludosphère](https://ludosphere.fr/@Ludovick)
🗣️ [Discord](https://discord.gg/ARk7tFjAdK)

💡 **Rejoignez l’aventure et façonnez le futur de FuncSug !** 🚀  


